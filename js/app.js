...
var showAnswerers = function (user) {
    ...
    $('.result .answerer').show();
    console.log(user);

    // Set the link in result
    var link = result.find('.link a');
    link.attr('href', user.link);
    link.text("Link to Answerer");
    ...
    dispName.text(user.display_name);

    // set profile image, currently not working
    //var profImg = result.find('.profile-image');
    //profImg.src = user.profile_image;

    return result;
};

function getAnswerers(tag) {
    // the parameters we need to pass in our request to StackOverflow's API
    var request = {
        tag: tag
    };
    ...
    $.ajax({
        ...
        .done(function (data) {
            ...
            $.each(data.items, function (i, item) {
                var answerer = showAnswerers(item.user);
